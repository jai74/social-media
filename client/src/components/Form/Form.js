...
const Form = ({ currentId, setCurrentId }) => {
    ...
    return (
        ...
        <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
            <Typography variant="h6">{currentId ? `Editing "${post.title}"` : 'Write your story!'}</Typography>
            <TextField name="creator" variant="outlined" label="Writer" fullWidth value={postData.creator} onChange={(e) => setPostData({ ...postData, creator: e.target.value })} />
